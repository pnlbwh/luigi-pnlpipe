...
#  THROUGH THE TERMINAL AND WISH DOE THEM TO BE DIFFERENT.                                    #
###############################################################################################
...
#bse.py -i "dwi/sub-${SUB_ID}_ses-${SES}_${DWI_IDENTIFIER}.nii.gz" -o INPUTS/b0.nii.gz || { echo "Error: bse.py failed."; exit 1; }
...
# organize outputscd /
#cd dwi/
#mv ../OUTPUTS/sub-${SUB_ID}_ses-${SES}_dir-80_desc-XcUnEdEp_dwi.nii.gz .
#mv ../OUTPUTS/sub-${SUB_ID}_ses-${SES}_dir-80_desc-XcUnEdEp_dwi.eddy_rotated_bvecs sub-${SUB_ID}_ses-${SES}_dir-80_desc-XcUnEdEp_dwi.bvec
